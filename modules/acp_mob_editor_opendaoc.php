...
//   - New mobs leave Brain empty on creation, matching how OpenDAoC's
//     own dumps leave it for standard NPCs (the server assigns a brain
//     itself; 'DOL.AI.Brain.StandardMobBrain' is not an OpenDAoC class).
//   - npctemplate linking (TemplateId int, searched/linked by mob.NPCTemplateID)
//     works the same as the DOL edition; OpenDAoC's npctemplate table has
//     more columns than this editor surfaces, but Spells/Styles/TetherRange/
//     ParryChance/EvadeChance/BlockChance/LeftHandSwingChance are still the
//     fields granted to a linked mob, same as before.
...
if (isset($_GET['action'])) {
    ...
    try {
        ...
        if ($action === 'get_mob_detail') {
            ...
            $stmt = $db->prepare(
                "SELECT mob.Mob_ID, mob.Name, mob.Level, mob.Model, mob.Race, mob.Size, mob.Speed,
                        ...
                        mob.FactionID, mob.RoamingRange, mob.MeleeDamageType, mob.BodyType,
        ...
        if ($action === 'update_mob' && $_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $factionId  = max(0,          (int)($_POST['faction_id']  ?? 0));
            ...
            $stmt = $db->prepare(
                "UPDATE mob SET Name=?,Level=?,Model=?,Size=?,AggroLevel=?,AggroRange=?,
                 ...
                 FactionID=?,RoamingRange=?,MeleeDamageType=?,BodyType=?,Gender=?,Flags=?,
            ...
            echo json_encode(['success'=>$stmt->execute([
                ...
                $factionId,$roaming,$dmgType,$bodyType,$gender,$flags,
                $npcTplId,$mobId,
            ])]);
            exit;
        }
        ...
        if ($action === 'add_mob' && $_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $stmt = $db->prepare(
                "INSERT INTO mob (Mob_ID,Name,X,Y,Z,Region,Realm,Level,Model,ClassType,Speed,Size,
        ...
        if ($action === 'group_spawn' && $_SERVER['REQUEST_METHOD']==='POST') {
            ...
            $st=$db->prepare("INSERT INTO mob (Mob_ID,Name,X,Y,Z,Region,Realm,Level,Model,ClassType,Speed,Size,RespawnInterval,Constitution,Dexterity,Strength,Quickness,Intelligence,Piety,Charisma,Empathy,AggroLevel,AggroRange,MeleeDamageType,BodyType,Flags,PackageID,LastTimeRowUpdated) VALUES (?,?,?,?,?,?,0,?,?,'DOL.GS.GameNPC',200,50,120,30,30,30,30,30,30,30,30,0,500,2,0,0,'mob_lab',NOW())");
            $ids=[];
            foreach($pos as $p){$gid='mob_lab_'.bin2hex(random_bytes(8));$st->execute([$gid,$name,$p[0],$p[1],$gz,$region,$level,$model]);$ids[]=$gid;}
    ...
    <div class="ep-tab-panel active" id="eptab-stats">
        ...
        <div class="ep-section">
            ...
            <div class="ep-grid">
                ...
                <div class="ep-field"><label>Faction ID</label><input id="ep-factionid" type="number" value="${mob.FactionID||0}" min="0"></div>
