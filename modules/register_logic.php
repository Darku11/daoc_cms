...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['register_submit'])) {
    ...
    if ($username !== '' && !preg_match('/^[A-Za-z0-9_-]+$/D', $username)) { $errors[] = t('register.error_username_chars', [], 'Username may only contain letters, numbers, underscores and hyphens.'); }
    ...
    if (empty($errors)) {
        try {
            ...
            if ($req_email) {
                ...
                $safeVerifyLink = htmlspecialchars($verifyLink, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
                ...
                <div style='max-width:600px; margin:auto; background:#111; border-top:3px solid #d4af37; padding:40px; text-align:center;'>
                    <h2 style='color:#d4af37; font-family:serif;'>Welcome to DAoC CMS, {$safeUsername}!</h2>
                    <p>Your account has been created. Please verify your email address.</p>
                    <div style='margin-top:30px;'>
                        <a href='{$safeVerifyLink}' style='background:#d4af37; color:#000; padding:12px 30px; text-decoration:none; font-weight:bold; text-transform:uppercase;'>
                            Verify Email
                        </a>
                    </div>
                </div>
