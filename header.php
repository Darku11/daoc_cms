...
(function(){
    ...
    function layout(){const items=getItems();items.forEach(el=>{el.style.display='';});moreMenu.innerHTML='';moreLabel.textContent=moreText;moreWrap.style.display='flex';moreWrap.style.visibility='hidden';const reserve=moreBtn.offsetWidth+20;moreWrap.style.display='none';moreWrap.style.visibility='';const available=nav.clientWidth;let used=0,start=-1;for(let i=0;i<items.length;i++){used+=items[i].offsetWidth;if(used>available-reserve){start=i;break;}}if(start<0)return;if(start>0&&!items[start].classList.contains('nav-sep')&&items[start-1].classList.contains('nav-sep'))start--;if(start>=items.length)return;const overflow=items.slice(start).filter(el=>el.tagName==='A');if(!overflow.length)return;overflow.forEach(link=>moreMenu.appendChild(link.cloneNode(true)));for(let i=start;i<items.length;i++)items[i].style.display='none';moreWrap.style.display='flex';}
    ...
    document.addEventListener('click',e=>{if(!moreWrap.contains(e.target)&&!moreMenu.contains(e.target))close();});let layoutFrame=0;function scheduleLayout(){close();if(layoutFrame)cancelAnimationFrame(layoutFrame);layoutFrame=requestAnimationFrame(()=>{layoutFrame=0;layout();});}window.addEventListener('resize',scheduleLayout);window.addEventListener('scroll',close,{passive:true});if(document.fonts?.ready)document.fonts.ready.then(scheduleLayout);window.addEventListener('load',scheduleLayout);layout();
})();
</script>
